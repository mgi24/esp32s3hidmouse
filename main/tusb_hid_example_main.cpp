...
static const tusb_desc_device_t custom_device_descriptor = {
    ...
    .bcdUSB             = 0x0200, // USB 2.0 (same as default TinyUSB to avoid compatibility issues)
    .bDeviceClass       = 0x00,
    .bDeviceSubClass    = 0x00,
    .bDeviceProtocol    = 0x00,
    .bMaxPacketSize0    = 64,     // 64 bytes (same as default TinyUSB to avoid compatibility issues)
...
static void parse_and_send_mouse(const char *line, uint32_t &recv_count, uint32_t &sent_count) {
  ...
  if (tud_mounted()) {
    ...
    if (tud_hid_n_ready(0)) {
      if (tud_hid_n_mouse_report(0, 0, (uint8_t)button, send_x, send_y, send_wheel, 0)) {
        sent_count++;
        accum_x = 0;
        accum_y = 0;
        accum_wheel = 0;
